...
- Following this guide, you will set up TPM2 unlocking which will automatically unlock your encrypted root partition during boot. This is a big convenience factor but would allow an attacker theoratically to break the encryption of this partition, if he/she steals the whole server. Encrypting the root partition makes sense nonetheless, since an attacker cannot just take out the internal drive, connect it to another PC and read out all the data in cleartext.
