...
1. Wait until everything is installed and it reports that Jellyfin was successfully installed
...
- `fail2ban` gets set up automatically and bans ip addresses that failed to log in 10 times within 30 minutes. You can unban an ip address by running `sudo fail2ban-client set jellyfin unbanip XX.XX.XX.XX`. This requires the Jellyfin log level to stay at its default `Information` since Jellyfin doesn't log failed logins on higher log levels. So please don't raise the log level in `/home/plex/jellyfin/config/logging.json` if you want to keep `fail2ban` working.
